...
#define Num 3 //Num of samples

// global variables
float timeout; // unit: us
float dist_min, dist_max, dist_raw, dist_all, dist_med, alpha; // unit: mm
...
// get a distance reading from the USS
  ...
  if(nowNum<Num){
    samples[nowNum]=dist_raw;
    dist_all+=dist_raw;
    dist_med=dist_raw;
  }
  else{
    dist_all-=samples[nowNum%Num];
    dist_all+=dist_raw;
    samples[nowNum%Num]=dist_raw;
    dist_med=dist_all/Num;
  }
  nowNum=nowNum+1;
  if(nowNum>2*Num){
    nowNum=nowNum-Num;
